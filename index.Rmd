...
# * select tweets containing the following terms: `corona|virus|covid|flu|social distancing|pandemic`
...
ignore_words = c("rt", "amp", "qt", "pu", "tag", "i'm", "it's", "i’m", "it’s", "lr", "li", "ag", "media", "status")

# load the files

file_dir = "~/Documents/code/congresstweets/data"
file_pattern = "2020-0[2-4]{1}-.*.json"
...
trump_tweets = get_tweets(year = 2020) %>%
  ...
  filter(grepl("2020-02|2020-03|2020-04", month)) %>%
...
tweets = bind_rows(congress_tweets, trump_tweets) %>%
  mutate(party = ifelse(party == "democrat", "democrats",
                 ifelse(party == "republican", "republicans",
                 ifelse(party == "independent", "independents", party))))

# define wordcloud function

plot_number = function(data, timescale = "week", start_date = NULL, palette = palette4, plot = "line") {
...
### April
```{r, fig.width=8, fig.height=12}
plot_sentiment(tweets, start_date = "2020-04-01", duration = 29)
```
...
### Mar 28 - Apr 3
```{r, fig.width=8, fig.height=8}
plot_sentiment(tweets, n_words = 10, start_date = "2020-03-28", duration = 6)
```

### Apr 4 - Apr 10
```{r, fig.width=8, fig.height=8}
plot_sentiment(tweets, n_words = 10, start_date = "2020-04-04", duration = 6)
```
...
### April
```{r, fig.width=8, fig.height=4}
p = plot_content(tweets, start_date = "2020-04-01", duration = 29, n_words = 100)
p$plot
p$data %>%
  filter(party %in% c("democrats", "republicans", "trump")) %>%
  group_by(party) %>%
...
### Mar 28 - Apr 3
```{r, fig.width=8, fig.height=4}
p = plot_content(tweets, start_date = "2020-03-28", duration = 6)
p$plot
p$data %>%
  filter(party %in% c("democrats", "republicans", "trump")) %>%
  group_by(party) %>%
...
### Apr 4 - Apr 10
```{r, fig.width=8, fig.height=4}
p = plot_content(tweets, start_date = "2020-04-04", duration = 6)
p$plot
p$data %>%
  filter(party %in% c("democrats", "republicans", "trump")) %>%
  group_by(party) %>%
...
## tweets
To browse all tweets, download [all_tweets.csv](all_tweets.csv)
```{r}
tweets %>%
  select(state_name, title, first, last, twitter_handle, party, gender, ethnicity, day, text) %>%
  arrange(twitter_handle) %>%
